...
import 'package:rockpaper/userscore.dart';
...
class _RPCAnimationState extends State<RPCAnimation>
  ...
  void nextpage() {
    Future.delayed(Duration(seconds: 2), () {
      return PageTransition().nextpage(context,
          object: BattleGround.rounds(
            userscore: userscore,
            cpuscore: cpuscore,
          ));
    });
  ...
  Widget medal_of_winner() {
    return AnimatedPositioned(
        onEnd: () {
          ...
          nextpage();
          //todo: call a function to go to another place and pass in score and round number after 2 seconds
          // constructor where
        },
        duration: Duration(seconds: 1),
    ...
    if (username == 'you') {
      return Stack(
        children: [
          paper(1),
          scissor(1),
          RotatedBox(quarterTurns: 2, child: rock(1))
        ], // user
      );
    } else {
      // CPU
